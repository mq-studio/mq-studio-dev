...
function getContentColor(category: string) {
  switch (category) {
    case 'feeling':
...
function renderHighlightedText(text: string, highlight?: string): { __html: string } {
    ...
    try {
      ...
      if (searchParams.category) {
        results = results.filter(
          (result) => result.content.category === searchParams.category
        );
      }
    } catch (error) {
      console.error('Error fetching search results:', error);
      // Results will remain empty array, showing no results state
    }
  }
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="max-w-2xl mx-auto mb-12">
          ...
          <Suspense fallback={<div className="h-14 bg-gray-100 rounded-lg animate-pulse" />}>
...
export async function generateMetadata({
  ...
  return {
    ...
    description: query
      ? `Search results for "${query}" across publications, artworks, musings, and projects.`
      : 'Search MQ Studio content across publications, artworks, musings, and projects.',
  };
}
